...
* 🧠 **LangGraph 多智能体协作**
  ...
  * 独创 **90 分放行机制**，打分不合格自动打回重写，确保发给 HR 的每一句话都是王炸。
...
### 1. 硬件与系统前置要求 ⚠️ (必看)
* **操作系统**：当前代码高度定制化，**强烈建议在 macOS 环境下运行**（底层强依赖 Mac 的 `osascript` 唤醒浏览器机制及 Mac 版 Word 的 PDF 转换引擎）。Windows/Linux 用户需自行修改底层的脚本执行逻辑。
...
2. 点击 **“创建企业自建应用”**，为应用起名（例如：`Auto-Job-Hunter`），点击创建。
3. 在左侧栏 **“凭证与基础信息”** 中复制 `App ID` 和 `App Secret`，填入 `.env` 文件。
4. 在左侧栏 **“开发配置” -> “权限管理”**，开通以下两个核心权限：
   * `查看、评论、编辑和管理多维表格` (bitable:app:read_write)
   * `查看、评论和导出多维表格` (bitable:app:readonly)
5. 在左侧栏 **“应用发布” -> “版本管理与发布”**，创建版本（如 `1.0.0`）并申请发布。
6. 🔑 **给机器人“发钥匙”（极其重要！防报错指南）**
   * 回到你转存的**飞书多维表格**页面。
   * 点击右上角 **“分享”** 或 **“添加协作者”**（带 `+` 号的小人图标）。
   * 搜索你刚才创建的应用名称（`Auto-Job-Hunter`），授予其 **“可编辑”** 权限并完成。*(注：若跳过此步，程序将疯狂报错 `403 Forbidden`！)*
...
* **BOSS 直聘**：
  ```bash
  python boss_scraper/boss_cookie_harvester.py
