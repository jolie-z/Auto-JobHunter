...
@click.option('-p', '--page', default=1, type=int, help='起始页码 (默认: 1)')

def export(
    keyword: str, city: str, count: int, page: int,  # 👈 新增了 page: int 参数
    salary: str | None, exp: str | None, degree: str | None,
    industry: str | None, scale: str | None, stage: str | None, job_type: str | None,
    ...
    例: boss export "golang" --city 杭州 -p 2 -n 15 -o jobs.csv
    """
    cred = require_auth()
    ...
    try:
        def _collect(c: BossClient) -> list[dict]:
            ...
            # 🌟 魔改点 1：把写死的 range(1, ...) 换成从我们指定的 page 开始算
            for pg in range(page, page + pages_needed):
                data = c.search_jobs(
                ...
                # 🌟 魔改点 2：如果是一次性翻多页，强制在中间插入休眠（防封控）
                if pages_needed > 1 and pg < page + pages_needed - 1:
                    sleep_time = random.randint(5, 12)
                    console.print(f"  [dim]🛡️ 内部翻页防封控：伪装人类操作，发呆 {sleep_time} 秒...[/dim]")
